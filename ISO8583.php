...
class ISO8583
{
    ...
    private $DATA_ELEMENT = array (
        ...
        60  => array('ans', 999,  self::VARIABLE_LENGTH),        //Reserved for national use
        ...
        125 => array('ans', 999,  self::VARIABLE_LENGTH),        //Network management information
        126 => array('ans', 6,   self::VARIABLE_LENGTH),        //Issuer trace id
        127 => array('ans', 999, self::VARIABLE_LENGTH),        //Reserved for private use
        128 => array('ans',   16,  self::FIXED_LENGTH)            //Message authentication code (MAC) field
    );
                    ...
                    else
                    {
                        ...
                        if (strlen($tmp) == $len )
                        {
                            ...
                            if (strlen($tmp2) == $num)
                            {
                                ...
                                {
                                    if ($key == 55) {
                                        $this->_data[$key] = ascii2hex(ltrim($tmp2));
                                    } else {
